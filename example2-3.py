...
async def open_freeform() -> dict:
    """Open Freeform application on macOS"""
    global freeform_window
    try:
        # Launch Freeform using subprocess
        subprocess.Popen(['open', '-a', 'Freeform'])
        time.sleep(2)  # Wait for the application to open
        
        # Use AppleScript to maximize the window
        apple_script = '''
        ...
        freeform_window = True
        
        return {
            "content": [
                TextContent(
                    type="text",
                    text="Freeform opened successfully and maximized"
                )
            ]
        }
    except Exception as e:
...
@mcp.tool()
async def create_board_in_freeform() -> dict:
    """Create a new board in Freeform"""
    global freeform_window
    try:
        if not freeform_window:
            return {
                "content": [
                    TextContent(
                        type="text",
        ...
        # Use AppleScript to create a new board
        apple_script = '''
        tell application "Freeform"
            activate
            tell application "System Events"
        ...
        '''
        subprocess.run(['osascript', '-e', apple_script])
        time.sleep(1)  # Wait for the new board to be created
        ...
        return {
            "content": [
                TextContent(
                    type="text",
                    text=f"Error creating board: {str(e)}"
                )
            ]
        }

@mcp.tool()
async def create_square_in_freeform() -> dict:
    """Create a square in Freeform """
    global freeform_window
    try:
        if not freeform_window:
            return {
        ...
        # Use AppleScript to activate Freeform and select the rectangle tool through the menu
        apple_script = '''
        tell application "Freeform"
            activate
            tell application "System Events"
        ...
        '''
        subprocess.run(['osascript', '-e', apple_script])
        time.sleep(1)  # Wait for the tool to be selected

        # Click and drag to resize using cliclick
        try:
            # Resize the square using cliclick
            top_left = "766,401"
            bottom_right = "915,551"
            expanded_bottom_right = "1065,701"  # Expanded size

            # Move to top left, drag to expanded bottom right
            subprocess.run(["cliclick", f"m:{bottom_right}"])
            time.sleep(0.1)
            subprocess.run(["cliclick", f"dd:{bottom_right}"])
            time.sleep(0.1)
            subprocess.run(["cliclick", f"du:{expanded_bottom_right}"])
            time.sleep(1)

            # # Step 3: Insert Text Inside the Square
            # text_position = "850,500"  # Centered text position
            # subprocess.run(["cliclick", f"m:{text_position}"])
            # time.sleep(0.1)
            # subprocess.run(["cliclick", "t:Enter your text here"])
            # time.sleep(0.1)

        except FileNotFoundError:
            print("Ensure cliclick is installed and accessible from terminal.")
        
        return {
            "content": [
                TextContent(
                    type="text",
                    text=f"Square created successfully"
                )
            ]
        }
    except Exception as e:
        return {
            "content": [
                TextContent(
                    type="text",
                    text=f"Error creating square: {str(e)}"
                )
            ]
        }
    
@mcp.tool()
async def write_text_in_square_in_freeform(text: str) -> dict:
    """Write text in a square in Freeform"""
    global freeform_window
    try:
        if not freeform_window:
            return {
        ...
        try:
            # Insert Text Inside the Square
            text_position = "850,500"  # Centered text position
            subprocess.run(["cliclick", f"m:{text_position}"])
            time.sleep(0.1)
            subprocess.run(["cliclick", f"t:{text}"])  # Use the provided text
            time.sleep(0.1)
            ...
            return {
                "content": [
                    TextContent(
                        type="text",
                        text="Error: cliclick is not installed or not accessible from terminal."
                    )
                ]
            }
        ...
        return {
            "content": [
                TextContent(
                    type="text",
                    text=f"Text '{text}' written successfully in the square"
                )
            ]
        }
    except Exception as e:
